...
class SlotExtractor
{
    const ORDER_ID_PATTERN = '/#([a-z|0-9]{5,12})\b/';

    const ORDER_NUMBER_PATTERN = '/order[\\s\\-#:=>]*(?:(?:num(?:ber)?|id)[\\s#:]*)?(?:is[\\s]*)?([A-Za-z0-9]{3,12})/i';

    const TRANSACTION_ID_PATTERN = '/\\b(txn[a-zA-Z0-9]{3,})\\b/i';

    const REFUND_REASON_PATTERNS = [
    ...
    public function extract(string $message): array
    {
        ...
        // order_number
        if (preg_match(self::ORDER_ID_PATTERN, $m, $matches)) {
            $slots['order_number'] = $matches[1];
        } elseif (preg_match(self::ORDER_NUMBER_PATTERN, $m, $matches)) {
            $slots['order_number'] = $matches[1];
        }

        // transaction_id
        if (preg_match(self::TRANSACTION_ID_PATTERN, $m, $matches)) {
            $slots['transaction_id'] = $matches[1];
        }

        // refund_reason
        foreach (self::REFUND_REASON_PATTERNS as $pattern) {
